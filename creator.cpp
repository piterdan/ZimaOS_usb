    ...
    localImagePath("")
{
    ...
    // New unified UI connections
    connect(ui->startButton, SIGNAL(clicked()),
            this, SLOT(startButtonClicked()));
    connect(ui->radioOnline, SIGNAL(toggled(bool)),
            this, SLOT(onSourceModeChanged()));
    connect(ui->radioLocal, SIGNAL(toggled(bool)),
            this, SLOT(onSourceModeChanged()));
    ...
    // Initialize new UI state
    onSourceModeChanged();  // Set initial visibility
...
Creator::~Creator()
{
    if (imageFile.isOpen() && state == STATE_DOWNLOADING_IMAGE) {
        ...
        imageFile.remove();
    } else if (state == STATE_WRITING_IMAGE || state == STATE_AUTO_WRITING_IMAGE) {
...
void Creator::projectImagesChanged(const QString& version)
{
    ...
    updateStartButtonState();  // Update start button when image changes
}
...
void Creator::savePreferredRemovableDevice(int idx)
{
    ...
    flashProgressBarText("");
    updateStartButtonState();  // Update start button when device changes
}
...
void Creator::handleFinishedDownload(const QByteArray &data)
{
    ...
    case STATE_DOWNLOADING_IMAGE:
        ...
        {
            bool checksumOk = isChecksumValid(imageHash.result().toHex());
            if (checksumOk)
            ...
            delete averageSpeed;
            averageSpeed = nullptr;

            // Auto-write after download if flag is set and checksum is valid
            if (autoWriteAfterDownload && checksumOk) {
                state = STATE_IDLE;
                reset();
                // Start writing after a short delay to allow UI to update
                QTimer::singleShot(500, this, SLOT(startWriting()));
            } else {
                reset();
                state = STATE_IDLE;
                autoWriteAfterDownload = false;
...
void Creator::writingSyncing()
{
    ...
    if (state == STATE_WRITING_IMAGE || state == STATE_AUTO_WRITING_IMAGE)
...
void Creator::writingFinished()
{
    ...
    if (state != STATE_IDLE) {
        ...
        autoWriteAfterDownload = false;
        state = STATE_IDLE;
        ui->startButton->setText(tr("Start"));  // Reset button text
    }
...
// ============== New methods for unified download + write workflow ==============

void Creator::startButtonClicked()
{
    // Check if operation is in progress - if so, cancel it
    if (state == STATE_DOWNLOADING_IMAGE || state == STATE_AUTO_WRITING_IMAGE || state == STATE_WRITING_IMAGE) {
        cancelOperation();
        return;
    }

    // Validate device is selected
    if (ui->removableDevicesComboBox->currentIndex() < 0 ||
        ui->removableDevicesComboBox->itemData(ui->removableDevicesComboBox->currentIndex()).isNull()) {
        QMessageBox::warning(this, tr("No Device"),
                           tr("Please select a target device."));
        return;
    }

    if (useLocalFile) {
        // Local file mode - directly write
        ...
        setImageFileName(localImagePath);
        startWriting();
    } else {
        // Online download mode - download then auto-write
        if (ui->projectSelectBox->currentIndex() < 0 || ui->imageSelectBox->currentIndex() < 0) {
            QMessageBox::warning(this, tr("No Image Selected"),
                               tr("Please select a project and image version."));
            return;
        }
        autoWriteAfterDownload = true;
        startDownload();
    }
}

void Creator::onSourceModeChanged()
{
    // This method handles switching between online download and local file modes
    bool isOnlineMode = ui->radioOnline->isChecked();

    // Show/hide appropriate widgets based on mode
    ui->onlineWidget->setVisible(isOnlineMode);
    ui->localWidget->setVisible(!isOnlineMode);

    // Update useLocalFile flag
    useLocalFile = !isOnlineMode;

    // Update start button state
    updateStartButtonState();
}

void Creator::updateStartButtonState()
{
    bool canStart = false;
    ...
    int idx = ui->removableDevicesComboBox->currentIndex();
    QString destination = ui->removableDevicesComboBox->itemData(idx).toString();
    bool hasDevice = !destination.isNull();

    if (useLocalFile) {
        // Local file mode: need file and device
        canStart = hasDevice && !localImagePath.isEmpty() && QFile::exists(localImagePath);
    } else {
        // Online mode: need project/image selection and device
        canStart = hasDevice &&
                   ui->projectSelectBox->currentIndex() >= 0 &&
                   ui->imageSelectBox->currentIndex() >= 0;
    }

    ui->startButton->setEnabled(canStart);
}

void Creator::selectLocalFile()
{
    QString fileName = QFileDialog::getOpenFileName(this,
        tr("Select image file"),
        settings.value("preferred/savedir", getDefaultSaveDir()).toString(),
        tr("Image files (*.img *.gz *.zip);;All files (*.*)"));
...
void Creator::startDownload()
{
    // Reuse existing download logic from downloadButtonClicked
    // but set autoWriteAfterDownload flag

    state = STATE_DOWNLOADING_IMAGE;
    disableControls(DISABLE_CONTROL_DOWNLOAD);

    // Update start button to show Cancel
...
void Creator::startWriting()
{
    ...
    state = STATE_AUTO_WRITING_IMAGE;
    privileges.SetRoot();    // root need for opening a device
...
void Creator::cancelOperation()
{
    if (state == STATE_DOWNLOADING_IMAGE) {
        ...
        reset();
    } else if (state == STATE_AUTO_WRITING_IMAGE || state == STATE_WRITING_IMAGE) {
        state = STATE_IDLE;
        privileges.SetUser();
        resetProgressBars();
        flashProgressBarText(tr("Writing canceled."));
