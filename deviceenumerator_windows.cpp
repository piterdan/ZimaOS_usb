...
// Helper function to check if a drive is removable using more reliable method
static bool isRemovableDevice(const QString &driveLetter)
{
    QString devicePath = "\\\\.\\" + driveLetter.left(2);
    qDebug() << "Checking if device is removable:" << devicePath;
    ...
    if (hDevice == INVALID_HANDLE_VALUE) {
        ...
        qDebug() << "Failed to open device" << devicePath << "Error:" << error;
        return false;
    }
    ...
    if (!result) {
        DWORD error = GetLastError();
        qDebug() << "DeviceIoControl failed for" << devicePath << "Error:" << error;
        CloseHandle(hDevice);
        return false;
    }

    STORAGE_DEVICE_DESCRIPTOR* desc = (STORAGE_DEVICE_DESCRIPTOR*)buffer;
    qDebug() << "Device" << devicePath << "RemovableMedia:" << desc->RemovableMedia << "BusType:" << desc->BusType;

    CloseHandle(hDevice);

    // Check if device is removable or on USB bus
    if (desc->RemovableMedia || desc->BusType == BusTypeUsb) {
        qDebug() << "Device" << devicePath << "is removable!";
        return true;
    }
...
QStringList DeviceEnumerator_windows::getRemovableDeviceNames() const
{
    ...
    qDebug() << "=== Starting device enumeration ===";
    ...
    while (*szDriveLetters != '\0') {
        QString driveLetter = QString::fromWCharArray(szDriveLetters);
        UINT driveType = GetDriveType(szDriveLetters);

        qDebug() << "Checking drive:" << driveLetter << "Type:" << driveType;

        // Check for DRIVE_REMOVABLE or use more reliable method for USB devices
        if (driveType == DRIVE_REMOVABLE) {
            qDebug() << "Found DRIVE_REMOVABLE:" << driveLetter;
            names << driveLetter;
        } else if (driveType == DRIVE_FIXED) {
            qDebug() << "Drive is DRIVE_FIXED, checking if it's USB...";
            if (isRemovableDevice(driveLetter)) {
                qDebug() << "USB device detected:" << driveLetter;
                names << driveLetter;
            }
        }
    ...
    qDebug() << "=== Found" << names.size() << "removable devices ===" << names;
    return names;
}
